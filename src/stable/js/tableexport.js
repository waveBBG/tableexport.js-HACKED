    ...
    if (typeof define === 'function' && define.amd) {
            ...
            self.selectors.each(function () {
                ...
                var $rows = $el.find('tbody').find('tr'),
                    ...
                    exporters = {
                        ...
                        copy: function (rdel) {
                            var colD = TableExport.prototype.copy.separator,
                                dataURL = $rows.map(function (i, val) {
                                    if (!!~ignoreRows.indexOf(i - thAdj) || $(val).is(ignoreCSS)) {
                                        return;
                                    }
                                ...
                                dataObject = TableExport.prototype.escapeHtml(dataURL),
                                myContent = TableExport.prototype.copy.buttonContent,
                                myClass = TableExport.prototype.copy.defaultClass;

                            var clipboard = new Clipboard('caption:not(.head) button.'+myClass);

                            clipboard.on('success', function(e) {
                                var rowsCnt = $rows.length - 2;
                                var content = "Copied " + rowsCnt + " row" + (rowsCnt > 1 ? "s" : "") + " to the clipboard.";
                                showCopyMessage('Table copied', content);
                                e.clearSelection();
                            });

                            clipboard.on('error', function() {
                                showCopyMessage('Press Ctrl+C to copy');
                            });
                ...
                function createObjButton(dataObject, myContent, myClass) {
                    var attr = myClass === 'copy'
                        ? "data-clipboard-text='" + dataObject + "'"
                        : "data-fileblob='" + dataObject + "'";
                    var exportButton = "<button " + attr + " class='" + bootstrapClass + bootstrapTheme + myClass + "'>" + myContent + "</button>";
                    checkCaption(exportButton);
                }

                /**
                 * Show message on Copy action
                 * @param title {String}
                 * @param content {String}
                 */
                function showCopyMessage(title, content) {
                    content = content ? ("<p>"+content+"</p>") : '';
                    var $container = $("<div class='table-export-copy-info'><h6>"+title+"</h6>"+content+"</div>");
                    $('body').append($container);
                    $container.fadeIn(100).delay(1500).fadeOut(500, function() {
                        $container.remove();
                    });
                }
            });

            $("button[data-fileblob]:not(.copy)")
                .off("click")
                .on("click", function () {
        ...
        TableExport.prototype = {
            ...
            defaults: {
                ...
                formats: ["copy", "csv", "xlsx"],           // (String[]), filetype(s) for the export, (default: ["xlsx", "csv", "copy"])
